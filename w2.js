...
document.getElementById('extract-w2-btn').addEventListener('click', async function() {
  const res = await fetch('https://tax-au-backend-production.up.railway.app/api/dashboard/'/dashboard/extract-w2', {
    method: 'POST',
    headers: { 'Authorization': 'Bearer ' + getToken() }
  });
  const data = await res.json();
...
async function loadW2Data() {
  const res = await fetch('https://tax-au-backend-production.up.railway.app/api/dashboard/'/dashboard/w2-data', {
    headers: { 'Authorization': 'Bearer ' + getToken() }
  });
  const data = await res.json();
